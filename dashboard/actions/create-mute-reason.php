<?php
  ...
  if(isset($_POST['submit'])){
    if(!existsBanReason($_POST['reason_name'])){
      ...
      $sql = "INSERT INTO ReportSystem_reasonsdb(TYPE,NAME,BAN_LENGTH) VALUES ('MUTE','".$_POST['reason_name']."','".$length."')";
      if(mysqli_query($conn,$sql)){
        $sql4 = "INSERT INTO staffcoreui_staff_activities(USER_ID,ACTIVITY_TEXT,PERFORMED) VALUES ('".$_SESSION['user_id']."','created a new Mute-Reason with the Name ".$_POST['reason_name']."','".$update."')";
        if(mysqli_query($conn,$sql4)){
          header("Location: ../admin/manager/mute/?success=Reason%20was%20created%21");
        } else {
          header("Location: ../admin/manager/mute/?success=Reason%20was%20created%21");
        }
      } else {
      ...
      header("Location: ../admin/manager/mute/?error=Reason%20already%20exists%21");
    }
  } else {
    header("Location: ../error/500/?return_url=".BASE_URL."dashboard/admin/manager/mute");
  }
